...
// INTERFACE
	...
	function addPanels(){
	    ...
	    // fix up chat
	    var rightPanel = $('#right-panel').css({right:'auto',top:'0px',bottom:'0px',height:'100%',marginLeft:'5px',width:PANEL_WIDTH+'px',left:'auto',float:'left',position:'relative',display:'inline-block'}).addClass('ttx-panel');
	    ...
	    if ($("#ttx-panels-room").length===0){
	    	 rightPanel.before('<div id="ttx-panels-room" class="ttx-panel" style="position:relative;z-index:3;margin-left:5px;overflow:hidden;float:left;height:100%;top:auto;display:inline-block;width:'+PANEL_WIDTH+'px;"><ul id="ttx-panels-room-tabs"></ul></div>');
	    }
	    ...
	    if ($("#ttx-panels-queue").length===0){
	    	 $('#right-panel').before('<div id="ttx-panels-queue" class="ttx-panel" style="position:relative;z-index:3;margin-left:5px;overflow:hidden;float:left;height:100%;display:inline-block;width:260px;"><ul id="ttx-panels-queue-tabs"></ul></div>');
	    }
		...
		panels = $('#ttx-panels');
	    	...
	    	$('#ttx-panels').sortable({axis:'x',forcePlaceholderSize:true,tolerance:'pointer',appendTo:document.body});
	    }
